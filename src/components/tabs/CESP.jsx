...
const CESP = () => {
  ...
  const [selectedProjectType, setSelectedProjectType] = useState(""); // State for the selected project type filter
  const [selectedGsDivision, setSelectedGsDivision] = useState(""); // State for the selected GS division filter
  const [filteredCards, setFilteredCards] = useState([]); // State for filtered card data
  const [cardsData, setCardsData] = useState([]); // State to store fetched cards from the backend (MongoDB)
  ...
  // Fetch card data from backend when component mounts
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const fullUrl = `${apiUrl}/api/cesp`;
        console.log("Fetching from:", fullUrl);

        const response = await fetch(fullUrl); // Fetching data from the API
        const data = await response.json(); // Converting response to JSON
        setCardsData(data); // Set fetched data to cardsData state
  ...
  return (
    <div className="mr-5 ml-5 flex flex-col items-center font-sans">
      ...
      <h1 className="text-4xl font-bold mb-5 mt-5">
        Community Engagement in Sponsorship Plan Project
      </h1>
      ...
      <div className="flex flex-col lg:flex-row w-full">
        ...
        <div className="lg:w-2/5  bg-gray-100 p-6 m-3 rounded-xl">
          <h2 className="text-xl font-bold mb-4">List of CESP Projects</h2>
